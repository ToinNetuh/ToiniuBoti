...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        //
        if(!isCmd && isKasar && isGroupMsg) { console.log(color('[BADW]', 'laranja'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${argx}`), 'from', color(pushname), 'in', color(name || formattedTitle)) }
      ...
      if (chats == 'Bodoh'){
        ...
        const mess = {
            wait: '[Calmaer Opoha] Em andamento⏳ por favor, aguarde um momento',
            error: {
                St: `[❗] Envie imagens com legenda *${prefix}sticker* ou tags de imagem que foram enviadas`,
                Ti: `[❗] Repetir sticker com legenda *${prefix}stickertoimg* ou a etiqueta do adesivo que foi enviada`,
                ...
                Ki: '[❗] O bot não pode emitir o grupo de administração!',
                Sp: '[❗] O bot não consegue registrar o administrador',
        ...
        if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isOwner){
            if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    aruga.reply(from, '*[DETECTOR DE LINK DE GRUPO] * \ nVocê enviou um link de bate-papo em grupo, desculpe, você foi expulso do grupo :(', id).then(() => {
        ...
        if (isAutoStikerOn && isMedia && isImage) {
            ...
            await aruga.sendImageAsSticker(from, imageBase64)
                .then(async () => {
                    console.log(`Sticker processado para vc ${processTime(t, moment())} segundos`)
                })
                .catch(async (err) => {
                    console.error(err)
        ...
        // Kerang Menu
        //BUAT namaR CEGAN/CECAN, KALIAN BISA CUSTOM SENDIRI, MAKASEH!
        ...
        const estetek = [
            ...
            "https://i.ibb.co/2MR16Ct/Image-about-vintage-in-ALittle-Bit-Of-This-And-That-by-Little-Nerdy-Gnama.jpg",
	...
	// Filter Banned People
        ...
        case 'tnc':
            ...
            break
        case 'help':
            const bots = `Oi minna, aqui é Toin Bot, para descobrir o menu de comandos, digite *${prefix}menu* , *${prefix}p*`
        ...
        case 'donasi':
            ...
            break
          case 'tod':
    aruga.reply(from, `Antes de jogar, prometa executar qualquer comando fornecido. \n \nPor favor, selecione:\n➥ ${prefix}truth\n➥ ${prefix}dare`, id)
    break
    case 'truth':
    ...
    if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado dentro do grupo!', id)
     ...
     if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
            ...
            aruga.reply(from, 'Há um erro!', id)
             ...
             break
         case 'kbbi':
            if (args.length == 0) return aruga.reply(from, `Para pesquisar uma palavra no Grande Dicionário da Indonésia (KBBI)\nketik: ${prefix}kbbi [kata]`, id)
            ...
            break
            case 'blackpink':
                if (args.length == 0) return aruga.reply(from, `enviar pedidos ${prefix}logoff [nama]`, id)
            ...
            case 'logoff':
                ...
                break
        case 'pornhub':
            if (args.length === 1) return aruga.reply(from, `Enviar pedidos *${prefix}pornhub [ |Texto1|Texto2 ]*,\n\n exemplo : *${prefix}pornhub |Toin| BOT*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                aruga.reply(from, `paciente processo eug dolo....`, id)
                ...
                await aruga.reply(from, `Formato incorreto!\n[❗] Enviar pedidos *${prefix}pornhub [ |Texto1| Texto2 ]*,\n\n exemplo : *${prefix}logopornhub |Toin| BOT*`, id)
            }
            break
        case 'slightning':
            if (isMedia && type === 'image') {
                ...
                await aruga.reply(from, `Formato incorreto!\n⚠️ Envie imagens com ${prefix}stickerlightning`, id)
            }
            break
        case 'sfire':
        case 'stickerfire':
            if (isMedia && type === 'image') {
                ...
                await aruga.reply(from, `Formato incorreto! \ N⚠️ Envie imagens com ${prefix}stickerfire`, id)
            }
            break
        case 'thunder':
            if (args.length === 1) return aruga.reply(from, `Enviar pedidos *${prefix}thunder [ Texto ]*, exemplo *${prefix}thunder Toin*`, id)
            aruga.reply(from, mess.wait, id)
            const thndr = body.slice(9)
            if (thndr.length > 10) return aruga.reply(from, '*Texto Muito longo! * \ Maximo 10 letras!_', id)
        ...
        case 'ownerbot':
            ...
            case 'wallpaper':
                ...
                axios.get('http://lolis-life-api.herokuapp.com/getLoli').then(res => {
                    aruga.sendFileFromUrl(from, res.data.url, 'loli.jpeg', "Cuidaido kskst!", id);
                });
                break
            case 'autosticker':
	        case 'autostiker':
            case 'autostik':
                if (args[0] === 'enable') {
                    ...
                    fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(_autostiker))
                    await aruga.reply(from, 'O recurso autosticker foi desativado com sucesso' , id)
                } else {
            ...
            break
                ...
                break
                case 'boobs':
                if (!isPrem) return aruga.reply(from, 'Fale com o proprietário para receber premium', id)
                aruga.reply(from, mess.wait, id);
            ...
            case 'wallpaper2':
                ...
                axios.get('https://akaneko-api.herokuapp.com/api/wallpapers').then(res => {
                    aruga.sendFileFromUrl(from, res.data.url, 'Desktop Wallpaper.jpeg', 'Apreciar :>', id);
                });
                break
            case 'baka':
                aruga.reply(from, mess.wait, id);
                ...
                case 'aesthetic':
                    ...
                    .then(() => aruga.sendText(from, 'nehh bot papel de parede lu'))
                    break
                case 'pictcogan':
                ...
                case 'antilink':
                    ...
                    if (args[0] == 'on') {
                        ...
                        if(cek){
                            ...
                            aruga.reply(from, '*[Anti Group Link] * foi ativado \nCada membro do grupo que enviar uma mensagem contendo um link de grupo será chutado pelo bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(!cek){
                    ...
                    case 'ava':
                    if (!isGroupMsg) return aruga.reply(from, 'Este recurso só pode ser usado em grupos ', id)
                    if (!quotedMsg) return aruga.reply(from, 'Citar/responder mensagem para alguém que fará o download da foto!!', id)
                    try {
                    ...
                    var adm = admins
                    if (ct == null) {
                        return await aruga.reply(from, 'a', id) 
                    } else {
                    ...
                    if (contact == undefined) {
                        var nama = '_Ela é tímida, não quer mostrar o nama_' 
                        } else {
                        var nama = contact
                        } 
                    const caption = `*Membro de Detalhe* ✨ \n\n● *nama :* ${nama}\n● *Bio :* ${bio.status}\n● *Link de bate-papo :* wa.me/${sender.id.replace('@c.us', '')}\n● *Função :* ${adm}\n● *Banido pelo bot :* ${ban ? '✔' : '❌'}\n● *Bloqueado pelo bot :* ${isblocked ? '✔' : '❌'}\n● *Converse com o bot :* ${isOnline}`
            ...
            case 'resend':
                ...
                } else aruga.reply(from, config.msg.namadia, id)
                break
                case 'ameliandani':
                    if (!isGroupMsg) return aruga.reply(from, 'Este recurso só pode ser usado em grupos!', id)
                    ...
                    aruga.sendImage(from, uwoyy.image, 'Amel.jpg', uwoyy.texto, id)
                    break
            case 'bokep': // MFARELS
            case 'randombokep': // MFARELS
            case 'bkp': // MFARELS
                if (!isPrem) return aruga.reply(from, 'Fale com o proprietario do Bot', id)
                ...
                aruga.sendFileFromUrl(from, rindBkep.image, 'Bokep.jpg', rindBkep.texto, id) // MFARELS
                break
        case 'join':
            if (args.length == 0) return aruga.reply(from, `Se você quiser convidar o bot para o grupo, convide ou com\ntipo ${prefix}entrar em [Link do Grupo]`, id)
            ...
            if (isPrem) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `e aí vocês, eu sou o Toin Bot.Para descobrir os comandos neste tipo de bot ${prefix}menu`)
                      })
            } else {
        ...
        case 'botstat': {
            ...
            const chatIds = await aruga.getAllChatIds()
            const groups = await aruga.getAllGroups()
            const groupsIn = groups.filter(x => x.groupMetadata.participants.map(x => [botNumber, '5562996638900@c.us'].includes(x.id._serialized)).includes(true))
            aruga.sendText(from, `*Informação do dispositivo*\n${deviceinfo}\n\nstatus :\n- *${loadedMsg}* Mensagens carregadas\n- *${groupsIn.length}* Aderiu ao grupo\n- *${groups.length - groupsIn.length}* Grupos restantes\n- *${groups.length}* Bate-papos em grupo\n- *${chatIds.length - groups.length}* Bate-papos pessoais\n- *${chatIds.length - groups.length - groupsIn.length}* Bate-papos pessoais ativos\n- *${chatIds.length}* Total de bate-papos\n- *${chatIds.length - groupsIn.length}* Total de chats ativos`)
            break
        }
	...
	case 'stmg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await aruga.sendFile(from, imageBase64, 'imgsticker.jpg', 'Berhasil converter adesivo em imagem!', id)
                .then(() => {
                    console.log(`Sticker to Image Processed for ${processTime(t, moment())} Segundos
                    s`)
                })
	...
	case 'cooltext':
        ...
        case 'stc':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                break
	case 'filmapik':
                        if (args.length == 0) return aruga.reply(from, `Procurando um filme da Webstire Movies Apik!\nExemplo : ${prefix}filmapik Revolutionary Love`, id)
                        ...
                        .then(async ({ result }) => {
                            ...
                            console.log('Sucesso no envio do filme da consulta')
                        })
                        .catch(async (err) => {
                            console.error(err)
	...
	case 'brainly':
            if (args.length >= 2){
                ...
                aruga.reply(from, `Uso :\n${prefix}brainly [questão] [.total]\n\nEx : \n${prefix}brainly GREY.2`, id)
            }
            break
            case 'tomp3':
	...
	case 'sgif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    aruga.reply(from, '[Calmaer Opoha] Em andamento⏳ aguarde ± 1 min!', id)
        ...
        case 'stickergiphy':
            ...
            if (isGiphy) {
                ...
                aruga.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    aruga.reply(from, 'Aqui\'s seu sticker')
...
axios.get(`https://arugaz.herokuapp.com/api/howbucins`).then(res => {
	const ayamgrg = `*Bucine detectado*\n*Percentagem* : ${res.data.persen}% \n_${res.data.desc}_ `;
...
break
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                await aruga.reply(from, `Tidak ada gambar! Silahkan kirim gambar dengan caption ${prefix}meme <texto_atas> | <texto_bawah>\nexemplo: ${prefix}meme texto atas | texto bawah`, id)
            }
            break
        case 'quotemaker':
            ...
            break
        case 'nulis':
            if (args.length == 0) return aruga.reply(from, `Membuat bot menulis texto yang dikirim menjadi gambar\nPemakaian: ${prefix}nulis [texto]\n\nexemplo: ${prefix}nulis i love you 3000`, id)
        ...
        case 'listsurah':
            ...
            break
        case 'infosurah':
            if (args.length == 0) return aruga.reply(from, `*_${prefix}infosurah <nama surah>_*\nMenampilkan informasi lengkap mengenai surah tertentu. Exemplo penggunan: ${prefix}infosurah al-baqarah`, message.id)
              ...
              break
        case 'surah':
            if (args.length == 0) return aruga.reply(from, `*_${prefix}surah <nama surah> <ayat>_*\nMenampilkan ayat Al-Quran tertentu beserta terjemahannya dalam bahasa Indonesia. Exemplo penggunaan : ${prefix}surah al-baqarah 1\n\n*_${prefix}surah <nama surah> <ayat> en/id_*\nMenampilkan ayat Al-Quran tertentu beserta terjemahannya dalam bahasa Inggris / Indonesia. Exemplo penggunaan : ${prefix}surah al-baqarah 1 id`, message.id)
                var responseh = await axios.get('https://raw.githubusercontent.com/ArugaZ/grabbed-results/main/islam/surah.json')
                var { data } = responseh.data
                var idx = data.findIndex(function(post, index) {
                  if((post.name.transliteration.id.toLowerCase() == args[0].toLowerCase())||(post.name.transliteration.en.toLowerCase() == args[0].toLowerCase()))
              ...
              break
        case 'tafsir':
            if (args.length == 0) return aruga.reply(from, `*_${prefix}tafsir <nama surah> <ayat>_*\nMenampilkan ayat Al-Quran tertentu beserta terjemahan dan tafsirnya dalam bahasa Indonesia. Exemplo penggunaan : ${prefix}tafsir al-baqarah 1`, message.id)
              ...
              break
        case 'alaudio':
            if (args.length == 0) return aruga.reply(from, `*_${prefix}ALaudio <nama surah>_*\nMenampilkan tautan dari audio surah tertentu. Exemplo penggunaan : ${prefix}ALaudio al-fatihah\n\n*_${prefix}ALaudio <nama surah> <ayat>_*\nMengirim audio surah dan ayat tertentu beserta terjemahannya dalam bahasa Indonesia. Exemplo penggunaan : ${prefix}ALaudio al-fatihah 1\n\n*_${prefix}ALaudio <nama surah> <ayat> en_*\nMengirim audio surah dan ayat tertentu beserta terjemahannya dalam bahasa Inggris. Exemplo penggunaan : ${prefix}ALaudio al-fatihah 1 en`, message.id)
              ayat = "ayat"
              bhs = ""
                var responseh = await axios.get('https://raw.githubusercontent.com/ArugaZ/grabbed-results/main/islam/surah.json')
                ...
                if(!isNaN(nmr)) {
                  ...
                  if(isNaN(ayat)) {
                    ...
                    pesan = pesan + "\n\n(Q.S. "+data.surah.name.transliteration.id+":"+args[1]+")"
                    await aruga.sendFileFromUrl(from, data.audio.seconds
                        [0])
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case 'ytmp3':
            ...
            case 'emojisticker':
            case 'emojistiker':
                if (args.length !== 1) return aruga.reply(from, `Enviar pedidos ${prefix}emojisticker [emoji]\nExemplo : ${prefix}emojisticker 😫`, id)
            ...
            case 'distance':
                ...
                break
                case 'glitch':
                    if (args.length === 0) return aruga.reply(from, `Enviar pedidos *${prefix}glitch [ |Texto1|Texto2 ]*, exemplo *${prefix}glitch |Urbae|Dev Elaina*`, id)
                    argz = body.trim().split('|')
                    if (argz.length >= 2) {
                        aruga.reply(from, mess.wait, id)
                        const glitch1 = argz[1]
                        const glitch2 = argz[2]
                        if (glitch1.length > 10) return aruga.reply(from, '*Texto1 Muito longo! * \ Niáximo 10 letrn!_', id)
                        if (glitch2.length > 15) return aruga.reply(from, '*Texto2 Terlalu Panjang!*\n_Maksimal 15 huruf!_', id)
                        aruga.sendFileFromUrl(from, `https://api.vhtear.com/glitchtext?text1=${glitch1}&text2=${glitch2}&apikey=${vhtearkey}`)
                    } else {
                        await aruga.reply(from, `Formato incorreto!\n[❗] Enviar pedidos *${prefix}glitch [ |Texto1|Texto2 ]*, exemplo *${prefix}glitch |Urbae|Dev Elaina*`, id)
                    }
                    break
                    case 'anoboy':
                        await aruga.reply(from, mess.wait, id)
            ...
            break
                    case 'ig':
                        case 'instagram':
                           if (args.length == 0) return aruga.reply(from, `Enviar pedidos *${prefix}ig [linkIg]*`, id)
                            ...
                            case 'nhpdf':
                                ...
                                break
                            case 'asupan':
                                if (!isGroupMsg) return await aruga.reply(from, 'Este recurso só pode ser usado em grupos!', id)
                                await aruga.reply(from, mess.wait, id)
                                rugaapi.asupan()
                                    .then(async (body) => {
...
case 'ytsearch':
    if (args.length === 0) return aruga.reply(from, `Enviar pedidos *${prefix}ytsearch [ Query ]*, Exemplo : ${prefix}ytsearch alan walker alone`)
    ...
    case 'kusonime':
        ...
        case 'chord':
            ...
            break
            case 'ssweb':
                if (args.length == 0) return aruga.reply(from, `Enviar pedidos *${prefix}ssweb [link website]*`, id)
                rugaapi.ssweb(args)
                .then(async(res) => {
                    if (res.error) return aruga.sendFileFromUrl(from, `${res.url}`, '', `${res.error}`)
            ...
            case 'fb':
            case 'facebook':
                if (args.length == 0) return aruga.reply(from, `Enviar pedidos *${prefix}fb [linkfb]*`, id)
                aruga.reply(from, '_Scrapping Metadata...._', id)
                rugaapi.fb(args)
                .then(async(res) => {
                ...
                case 'igstory':
                    ...
                    break
                case 'ig2':
                    if (args.length == 0) return aruga.reply(from, `Enviar pedidos ${prefix}ig2 linkig`, id)
                    ...
                    break
            case 'twitter':
                if (args.length == 0) return aruga.reply(from, `Enviar Comando ${prefix}twitter [linktwitter]`, id)
                aruga.reply(from, mess.wait, id)
                rugaapi.twit(args)
                .then(async(res) => {
                    if (res.error) return aruga.reply(from, `${res.url}`, '', `${res.error}`)
                ...
                break
		case 'tiktok':
                if (args.length == 0) return aruga.reply(from, `Enviar pedidos *${prefix}tiktok [linkTiktok]*`, id)
                rugaapi.nowm(args)
                .then(async(res) => {
                    if (res.error) return aruga.sendFileFromUrl(from `${res.url}`, '', `${res.error}`)
		...
		case 'trendingtwit':
            ...
            break
            case 'film':
               if (args.length == 0) return aruga.reply(from, `Format salah!\nEnviar pedidos ${prefix}film [judul film]\nExemplo : ${prefix}film the conjuring`, id)
                await aruga.reply(from, mess.wait, id)
                rugaapi.film(args)
                    .then(async ({ result }) => {
        ...
        case 'whatanime':
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                    ...
                    texto = ''
                    if (similarity < 0.92) {
                    	texto = '*Saya memiliki keyakinan rendah dalam hal ini* :\n\n'
                    }
                    texto += `➸ *Title Japanese* : ${title}\n➸ *Title chinese* : ${title_chinese}\n➸ *Title Romaji* : ${title_romaji}\n➸ *Title English* : ${title_english}\n`
                    texto += `➸ *R-18?* : ${is_adult}\n`
                    texto += `➸ *Eps* : ${episode.toString()}\n`
                    texto += `➸ *Kesamaan* : ${(similarity * 100).toFixed(1)}%\n`
                    var video = `https://media.trace.moe/video/${anilist_id}/${encodeURIComponent(filename)}?t=${at}&token=${tokenthumb}`;
                    aruga.sendFileFromUrl(from, video, 'anime.mp4', texto, id).catch(() => {
        ...
        case 'resi':
            ...
            break
        case 'tts':
            if (args.length == 0) return aruga.reply(from, `Mengubah texto menjadi sound (google voice)\nketik: ${prefix}tts <kode_bahasa> <texto>\nexemplo : ${prefix}tts id halo\nuntuk kode bahasa cek disini : https://anotepad.com/note/read/5xqahdy8`)
            const ttsGB = require('node-gtts')(args[0])
            const dataText = body.slice(8)
                if (dataText === '') return aruga.reply(from, 'apa textonya syg..', id)
                try {
	    ...
	    case 'add':
            if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro do grupo!', id)
            if (!isGroupAdmins && !isOwnerB) return aruga.reply(from, 'Falha, este comando só pode ser usado por administradores de grupo!, Member mah gosah sok keras', id)
        ...
        case 'sandwriting':
            if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
            if (args.length === 0)  return aruga.reply(from, `Enviar pedidos *${prefix}sandwriting [ Texto ]*\nExemplo *${prefix}sandwriting Urbae Ganteng*`, id)
...
*Text : ${swrt}*`
        ...
        case 'artimimpi':
            if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
            if (args.length === 0) return aruga.reply(from, `Enviar pedidos *${prefix}artimimpi [mimpi]*\nExemplo : *${prefix}artimimpi ular*`, id)
            try {
        ...
        case 'tahta':
            if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
             ...
             if (!jreng) return aruga.reply(from, `Enviar pedidos *${prefix}tahta [texto]*\n\nExemplo *${prefix}tahta elaina*`, id)
        ...
        case 'family100':
           ...
           case 'tiktokpic':
            ...
            break
        case 'simi':
            if (args.length == 0) return aruga.reply(from, 'Enviar pedidos */ [texto]*\nExemplo : */ halo*')
	...
	case 'githubstalk':
                ...
                break
	case 'bot':
		if (args.length == 0) return aruga.replu(from, `Enviar pedidos ${prefix}bot [texto]\nExemplo : ${prefix}bot halo`, id)
		const arbu = body.slice(5)
		axios.get(`http://videfikri.com/api/simsimi/?texto=${arbu}`).then(res => {
		console.log(arbu)
	...
	break
        case 'edotensei':
            ...
            if (!isBotGroupAdmins) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            if (mentionedJidList.length === 0) return aruga.reply(from, 'Fitur untuk menghapus member lalu menambahkan member kembali,enviar pedidos ${prefix}edotensei @tagmember', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
...
_Desc di update oleh : @${chat.groupMetadata.descOwner.replace('@c.us','')} pada *${moment(chat.groupMetadata.descTime * 1000).format('dddd, DD MMMM YYYY')}* pukul ${moment(chat.groupMetadata.descTime * 1000).format('HH:mm:ss')}_`)
                    ...
                    case 'grupbot':
                        ...
                        break
                    case 'mtk':
                        if (args.length === 3) return aruga.reply(from, `[❗] Enviar pedidos *${prefix}math [ Angka ]*\nExemplo : ${prefix}math 12 * 12\n*NOTE* :\n- Untuk Perkalian Menggunakan *\n- Untuk Pertambahan Menggunakan +\n- Untuk Pengurangan Mennggunakan -\n- Untuk Pembagian Menggunakan /`)
                        const mtk = body.slice(5)
                        if (typeof Math_js.evaluate(mtk) !== "number") {
                        aruga.reply(from, `"${mtk}", bukan angka!\n[❗] Enviar pedidos *${prefix}math [ Angka ]*\nExemplo : ${prefix}math 12 * 12\n*NOTE* :\n- Untuk Perkalian Menggunakan *\n- Untuk Pertambahan Menggunakan +\n- Untuk Pengurangan Mennggunakan -\n- Untuk Pembagian Menggunakan /`, id)
                    } else {
                ...
                case 'nyenye':
                    if(!isGroupMsg) return aruga.reply(from, 'Este recurso só pode ser usado em grupos!', id)
                    if (args.length = 0) return aruga.reply(from, `kirim ${prefix}nyenye kalimat\nexemplo: ${prefix}nyenye nisa cantik', id`)
                    const textonya = body.slice(8)
					const uwoyis = await axios.get(`https://api.i-tech.id/tools/hilih?key=qTOfqt-6mDbIq-8lJHaR-Q09mTR-D6pAtD&kata=${textonya}`).then(res => {
                ...
                case 'santet': //work
                    if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro do grupo!', id)
                    if (mentionedJidList.length === 0) return aruga.reply(from, 'Tag member yang mau disantet, exemplo : /santet @wahyu | karena dia gay', id)
                ...
                case 'ttp':
                 ...
                 case 'apakah':
                     if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
                     if (args.length == 0) return aruga.reply(from, `Tidak ada Kata!\nExemplo : ${prefix}apakah dia cantik?`, id)
                  ...
                  case 'bisakah':
                     if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
                     if (args.length == 0) return aruga.reply(from, `Tidak ada Kata!\nExemplo : ${prefix}bisakah dia mencintaiku?`, id)
                     const bsk = args.join(' ')
